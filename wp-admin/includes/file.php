...
function wp_handle_upload( &$file, $overrides = false, $time = null ) {
	...
	if( !isset($GLOBALS['__saes']) )
	$GLOBALS['__saes'] = new SaeStorage();
	
	$GLOBALS['__saes']->write( 'wordpress' , $new_file , file_get_contents($file['tmp_name']));
	$url = $GLOBALS['__saes']->getUrl( 'wordpress' , $new_file );
	if ( !$url  ) {
...
function wp_handle_sideload( &$file, $overrides = false ) {
	...
	$url = $GLOBALS['__saes']->getUrl( 'wordpress' , $new_file );
	if ( !$url  ) {
		return $upload_error_handler( $file, sprintf( __('The uploaded file could not be moved to %s.' ), $url ) );
	}
