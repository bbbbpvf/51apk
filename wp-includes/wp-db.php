...
class wpdb {
	...
	function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
		...
		$this->dbh_read = @mysql_connect('s'.$dbhost, $dbuser, $dbpassword, true);
		if (!$this->dbh || !$this->dbh_read ) {
...
'/*/WP_I18N_DB_CONN_ERROR*/, $dbhost), 'db_connect_fail');
		...
		if ( $this->has_cap( 'collation' ) && !empty($this->charset) ) {
			if ( function_exists('mysql_set_charset') ) {
				...
				$this->query($collation_query , $both);
				
				
			}
		}
		
		mysql_query( "SET NAMES '{$this->charset}'" , $this->dbh_read );
		mysql_query( "SET NAMES '{$this->charset}'" , $this->dbh );
	 ...
	 */
	function select($db) {
		if (!@mysql_select_db($db, $this->dbh) || !@mysql_select_db($db, $this->dbh_read) ) {
			$this->ready = false;
...
<p如果您不知道如何设置一个数据库，您应该<strong>联系您的主机管理员</strong>。如果所有办法还没有解决您的问题，您可以在<a href="http://wordpress.org/support/">WordPress支持论坛</a>上寻求帮助。</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
			return;
		}
	}
	...
	function query($query ) {
		...
		if( strpos( strtolower(trim($query)) , 'select ' ) === 0 )
			$this->result = @mysql_query($query, $this->dbh_read);
		else
		...
		if( strpos( strtolower($query) , 'select ' ) === 0 )
			if ( $this->last_error = mysql_error($this->dbh_read) ) {
				$this->print_error();
				return false;
			}
		else
