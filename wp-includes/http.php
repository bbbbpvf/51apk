...
/* modified for sae by easychen@gmail.com  */
function sae_signature( $url , $timstamp  )
{
 $content="FetchUrl";
 $content.=$url;
 $content.="TimeStamp";
 $content.=$timstamp;
 $content.="AccessKey";
 $content.=$_SERVER['HTTP_ACCESSKEY'];
 return base64_encode( hash_hmac( 'sha256' , $content , $_SERVER['HTTP_SECRETKEY'] , true ));
}
...
class WP_Http_Curl {
	...
	function request($url, $args = array()) {
		...
		// change to sae fetch url location
        curl_setopt( $handle , CURLOPT_URL , SAE_FETCHURL_SERVICE_ADDRESS);
		...
		*/
        ...
        $r['headers']['FetchUrl'] = $url;
        $r['headers']['TimeStamp'] = $timest;
        $r['headers']['AccessKey'] = $_SERVER['HTTP_ACCESSKEY'];
        $r['headers']['Signature'] = sae_signature( $url , $timest );
